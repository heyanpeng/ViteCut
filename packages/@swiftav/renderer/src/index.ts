...
export interface RenderLoopOptions {
   ...
   * 上层可以在回调中使用 @swiftav/canvas 在该 canvas 上绘制每一帧。
   */
  ...
  /**
   * 每一帧渲染回调。
   * 调用方在该函数内根据 time 使用 @swiftav/canvas / @swiftav/project 等完成画面绘制。
   */
 ...
 * 通用的离线渲染循环：
 *
 * - 使用 @swiftav/media 创建基于 canvas 的视频输出
 * - 按给定 fps 迭代时间
 * - 对每一帧调用 renderFrame，让上层去拼 timeline + canvas + project
 * - 最后调用 finalize 完成编码
 *
 * 该模块不直接依赖 @swiftav/canvas / @swiftav/project，以保持包之间的职责独立。
 */
